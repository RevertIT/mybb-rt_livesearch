...
function misc_start(): void
{
	...
	if ($mybb->get_input('action') === Core::get_plugin_info('prefix') && $mybb->get_input('load') === 'modal')
	{
...
function search_results_thread(): void
{
	...
	if ($mybb->get_input('ext') === Core::get_plugin_info('prefix'))
	{
		...
		{
			$thread_cache = array_slice($thread_cache, 0, (int) $mybb->settings['rt_livesearch_total_results'], true);
		...
		{
			$view_all = $lang->sprintf($lang->rt_livesearch_view_all, $total_threads);
		}
